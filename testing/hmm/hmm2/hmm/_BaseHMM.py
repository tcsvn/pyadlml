...
class _BaseHMM(object):
    ...
    def forwardbackward(self,observations, cache=False):
        ...
        '''
        if (cache==False):
            self.mapB(observations)
        
        alpha = self.calcalpha(observations)
        return numpy.log(sum(alpha[-1]))
    
    def calcalpha(self,observations):
        '''
    ...
    def calcbeta(self,observations):
        '''
...
#        path = numpy.zeros((len(observations)),dtype=self.precision) ### 2012-11-17 - BUG FIX: wrong reinitialization destroyed the last state in the path
    ...
    def calcxi(self,observations,alpha=None,beta=None):
        '''
    ...
    def reestimateA(self,observations,xi,gamma):
        '''
    ...
    def _calcstats(self,observations):
        ...
        stats['alpha'] = self.calcalpha(observations)
        stats['beta'] = self.calcbeta(observations)
        stats['xi'] = self.calcxi(observations,stats['alpha'],stats['beta'])
        stats['gamma'] = self.calcgamma(stats['xi'],len(observations))
    ...
    def _reestimate(self,stats,observations):
        ...
        new_model['A'] = self.reestimateA(observations,stats['xi'],stats['gamma'])
