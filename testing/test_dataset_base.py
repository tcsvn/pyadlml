...
class TestDatasetBase(unittest.TestCase):
    ...
    def test_fetch(self):
        ...
        self.data = self.fetch_method(keep_original=False, cache=False)
        clear_data_home()
        self.data = self.fetch_method(keep_original=True, cache=False)
        clear_data_home()
        self.data = self.fetch_method(keep_original=False, cache=True)
        clear_data_home()
        self.data = self.fetch_method(keep_original=True, cache=True)
        clear_data_home()
        set_data_home(self.data_home)

    def test_activities_loaded(self):
        for df_activity_attr in self.df_activity_attrs:
    ...
    def test_stats_activities(self):
        ...
        for df_activity_attr in self.df_activity_attrs:
            ...
            assert len(act_count) == self.num_activities
            ...
            assert len(act_dist.columns) == self.num_activities
            assert len(act_dist) == 100
