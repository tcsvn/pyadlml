...
class TestAmsterdamDataset(unittest.TestCase):
    def setUp(self):
        set_data_home('/tmp/pyadlml_testing')
        self.data = fetch_amsterdam(keep_original=False, cache=False)

    def test_fetch(self):
        set_data_home('/tmp/pyadlml_testing/test_fetch')
        self.data = fetch_amsterdam(keep_original=False, cache=False)
        clear_data_home()
        self.data = fetch_amsterdam(keep_original=True, cache=False)
        clear_data_home()
        self.data = fetch_amsterdam(keep_original=False, cache=True)
        clear_data_home()
        self.data = fetch_amsterdam(keep_original=True, cache=True)
        clear_data_home()
        set_data_home('/tmp/pyadlml_testing')

    def test_activities_loaded(self):
        df_act = getattr(self.data, 'df_activities')
