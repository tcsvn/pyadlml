...
class RNN(nn.Module):
    ...
    def reset_weights(self):
        ...
        # input and output are provided as (batch, seq, feature)
...
class RNNClassifier(NeuralNetClassifier):
    ...
    def get_loss(self, y_pred, y_true, X=None, training=False):
        ...
        y_true : torch tensorr
          True target values.
        X : input data, compatible with skorch.dataset.Dataset
        """
        if self.module_.seq_type == 'many-to-many':
            y_true = torch.flatten(to_tensor(y_true, device=self.device),
            ...
            y_true = y_true.view(-1).type(torch.LongTensor)
        else:
