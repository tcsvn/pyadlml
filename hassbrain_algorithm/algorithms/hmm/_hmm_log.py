...
class HMM_log(HiddenMarkovModel):
    ...
    def new_pi_mean(self, new_pi_arr, len_seqs):
        ...
        :return:
        """
        # todo find theoretical basis for calculating log_mean
        #print('~'*100)
        #print(new_pi_arr)
        #print(np.apnew_pi_arr))
        #print('--'*100)
        return new_pi_arr.sum(axis=0)
    ...
    def q_energy_function(self, seq, gamma, xi):
        ...
        first_term = 0.0
        for k in range(0, K):
            first_term += np.exp(gamma[0][k]) * self._pi[k]
        ...
        for n in range(0, N-1):
            for j in range(0, K):
                for k in range(0, K):
                    second_term += np.exp(xi[n][j][k]) * self._A[j][k]

        # compute third term
        third_term = 0.0
        for n in range(0, N):
            for k, zn in enumerate(self._z):
                third_term += np.exp(gamma[n][k]) \
                              * self.prob_x_given_z(seq[n], zn)
