...
class HiddenMarkovModel():
    ...
    def norm_forward(self, seq, cn):
        ...
        """
        print(cn)
        alpha = np.zeros((len(self._z), len(seq)))
        # eq (13.37)
        for idx, zn in enumerate(self._z):
            alpha[idx][0] = \
                self.prob_pi(zn)*self.prob_x_given_z(seq[0],zn)*cn[0]
        ...
        for t in range(1,len(seq)):
            for idx, zn in enumerate(self._z):
                ...
                for idx_znm1, znm1 in enumerate(self._z):
                    ...
                    s += "(" + str(alpha[idx_znm1][t-1])+ "*" + str(self.prob_za_given_zb(zn, znm1)) + ")+"
                    sum += alpha[idx_znm1][t-1]\
                           *self.prob_za_given_zb(zn, znm1)
